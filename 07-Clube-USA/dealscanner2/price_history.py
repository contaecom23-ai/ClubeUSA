...
def get_trend(asin) -> str:
    """
    Retorna tendencia de preco com base nas ultimas 5 entradas:
      '↓' caindo  (queda > 3%)
      '↑' subindo (alta > 3%)
      '→' estavel
    """
    ...
    if asin not in h:
        return "→"
    entries = [e for e in h[asin].get("entries", []) if e.get("price") is not None]
    if len(entries) < 3:
        return "→"
    recent = [e["price"] for e in entries[-5:]]
