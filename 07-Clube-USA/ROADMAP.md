...
- [x] **0.1** Cadastro + perfil mínimo + email confirmado
  - Backend FastAPI: registro, login (JWT 7d), confirmação de email, perfil get/update
  - Segurança: bcrypt, rate-limit (5/min registro, 10/min login), CORS restrito, security headers
  - Isolamento multi-tenant: user_id sempre do token, nunca do cliente; 404 em recurso alheio
  - Schema SQL Supabase com triggers updated_at e índices
  - Suite de testes com mocks (sem chamada real ao Supabase)
  - Frontend HTML: registro, login, confirmação, perfil
  - PR: feat/fase-0-1-cadastro-perfil
